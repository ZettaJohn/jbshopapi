...
const model = {
    find_user(username, pass) {
        return new Promise((resolve, reject) => {
            nameFN = "find_user"
            nameTB = "jb_user"
            sql_select = `SELECT * FROM jb_user WHERE id_user LIKE ${username} AND pass_user LIKE ${pass} `
            sqlQuery.sql_select(connMyDB, nameTB, nameFN, sql_select)
            .then((response)=>{
    ...
    async create_new_item(newBarcode, inData, callback) {
        ...
        inData.forEach((val, i) => {
            ...
            sql_insert = "INSERT INTO item_master \
            (barcode \
            ,item_name\
            ,old_price\
            ,new_price\
    ...
    async report_sale_order(stDate, enDate, callback) {
        ...
        WHERE CONVERT(create_date,char(10)) BETWEEN '" + stDate + "' AND '" + enDate + "' \
